...
import roomescape.service.stub.FakeReservationQueryRepository;
...
class ReservationQueryServiceTest {
    ...
    void setUp() {
        ...
        reservationQueryService = new ReservationQueryService(
                new FakeReservationQueryRepository(reservationRepository, reservationTimeRepository)
        );
    }
    ...
    void getAll_success() {
        ...
        reservationRepository.save(Reservation.createNew("쿠다", LocalDate.now().plusDays(1), time.getId()));
    ...
    void getAllByName_success() {
        ReservationTime time = saveReservationTime();
        reservationRepository.save(Reservation.createNew("쿠다", LocalDate.now().plusDays(1), time.getId()));
        reservationRepository.save(Reservation.createNew("피케이", LocalDate.now().plusDays(2), time.getId()));
